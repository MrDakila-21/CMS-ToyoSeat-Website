...
function initAnnouncementManagement() {
    ...
    // Refresh data when window gets focus (coming back from other apps)
    ...
    window.addEventListener('announcementDataChanged', function() {
        console.log('Data change event detected, refreshing...');
        loadDataFromServer();
    });
    
    // Also refresh after a short delay when modals close (as a fallback)
    if (addModalElement) {
        addModalElement.addEventListener('hidden.bs.modal', function() {
            console.log('Add modal closed, refreshing data...');
            setTimeout(() => {
                loadDataFromServer();
            }, 100);
        });
    ...
    if (directUploadModalElement) {
        directUploadModalElement.addEventListener('hidden.bs.modal', function() {
            console.log('Upload modal closed, refreshing data...');
            setTimeout(() => {
                loadDataFromServer();
            }, 100);
        });
    ...
    try {
        ...
        // Add a cache buster to each item's image_url if not already present
        allData = allData.map(item => {
            if (item.image_url) {
                // Add or update cache buster
                const separator = item.image_url.includes('?') ? '&' : '?';
                item.image_url = `${item.image_url}${separator}_cb=${timestamp}`;
            }
            return item;
        });
        ...
        if (pageData.length === 0) {
            ...
            pageData.forEach(item => {
                const imageHtml = item.image_url 
                    ? `<img src="${item.image_url}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 6px;" loading="lazy" onerror="this.onerror=null; this.src='/images/default-image.png';">`
